...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        ...
        UIImage *previousButtonImage = [UIImage imageForResourcePath:[NSString stringWithFormat:arrowPathFormat, @"Left"] ofType:@"png"];
        UIImage *nextButtonImage = [UIImage imageForResourcePath:[NSString stringWithFormat:arrowPathFormat, @"Right"] ofType:@"png"];
...
- (void)performLayout {
    ...
    if (_enableGrid) {
        hasItems = YES;
        [items addObject:[[UIBarButtonItem alloc] initWithImage:[UIImage imageForResourcePath:@"KSMWPhotoBrowser.bundle/UIBarButtonItemGrid" ofType:@"png"] style:UIBarButtonItemStylePlain target:self action:@selector(showGridAnimated)]];
    } else {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            if (page.displayingVideo) {
                ...
                [playButton setImage:[UIImage imageForResourcePath:@"KSMWPhotoBrowser.bundle/PlayButtonOverlayLarge" ofType:@"png"] forState:UIControlStateNormal];
                [playButton setImage:[UIImage imageForResourcePath:@"KSMWPhotoBrowser.bundle/PlayButtonOverlayLargeTap" ofType:@"png"] forState:UIControlStateHighlighted];
            ...
            if (self.displaySelectionButtons) {
                ...
                [selectedButton setImage:[UIImage imageForResourcePath:@"KSMWPhotoBrowser.bundle/ImageSelectedOff" ofType:@"png"] forState:UIControlStateNormal];
                ...
                if (self.customImageSelectedIconName) {
                    ...
                    selectedOnImage = [UIImage imageForResourcePath:@"KSMWPhotoBrowser.bundle/ImageSelectedOn" ofType:@"png"];
                }
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    [UIView animateWithDuration:animationDuration animations:^(void) {
        ...
        if (hidden) self.toolbar.frame = CGRectOffset(self->_toolbar.frame, 0, animatonOffset);
            ...
            if(photo.isVideo){
                if ([photo respondsToSelector:@selector(getVideoURL:)]) {
                    [photo getVideoURL:^(NSURL *url) {
                        ...
                        NSString *videoDirectory = [NSHomeDirectory() stringByAppendingFormat:@"/Documents/Video"];
                        NSFileManager *fileManager = [NSFileManager defaultManager];
                        BOOL isDir;
                        if(![fileManager fileExistsAtPath:videoDirectory isDirectory:&isDir]){
                            [fileManager createDirectoryAtPath:videoDirectory withIntermediateDirectories:YES attributes:nil error:nil];
                        }
                        
                        NSString *filePath = [videoDirectory stringByAppendingFormat:@"/%@.mp4", videoName];
